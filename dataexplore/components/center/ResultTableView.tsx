...
const ResultTableView: React.FC<ResultTableViewProps> = ({ tableData, htmlOutput }) => {
      ...
      <html>
        <head>
          ...
          <style>
            ...
            table:not(.Rtable1) { border-collapse: collapse; width: 100%; margin-top: 20px; font-size: 10pt; }
            table:not(.Rtable1) th { border-bottom: 2px solid #333; padding: 8px; text-align: left; font-weight: bold; background: #f9f9f9; }
            table:not(.Rtable1) td { border-bottom: 1px solid #eee; padding: 8px; }
          </style>
        </head>
        <body>
          ${htmlOutput}
  ...
  // If we have HTML output (e.g. from table1 or kable), render it directly
  if (htmlOutput) {
    return (
      <div className="w-full h-full flex flex-col bg-white p-2 md:p-4 overflow-hidden animate-in fade-in duration-300">
        ...
        <div className="flex-1 bg-white overflow-auto custom-scrollbar p-2 md:p-8" ref={printRef}>
          <style dangerouslySetInnerHTML={{ __html: `
            ...
            .Rtable1 .firstrow .rowlabel { font-weight: 400; font-style: italic; }
            .Rtable1 .stratlabel { font-weight: 700; border-bottom: 1.25px solid #1a1f24; }
            .Rtable1 caption { margin-bottom: 1.5em; font-weight: 800; font-size: 18px; text-transform: uppercase; tracking: 0.05em; color: #1a1f24; }
            
            /* Professional Styling for kable (Standard HTML Tables) */
            table:not(.Rtable1) { 
              border-collapse: collapse; 
              width: 100%; 
              margin: 1.5em 0; 
            ...
            table:not(.Rtable1) tbody tr:nth-of-type(even) {
  ...
  // Fallback to standard grid for JSON data (legacy or custom objects)
  return (
