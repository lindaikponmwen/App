...
const RTerminal: React.FC = () => {
  ...
  const runCode = useCallback(async (code: string, source: RunCodeSource = 'console') => {
    ...
    try {
        ...
        // Heuristic to detect table1 HTML output in stdout
        let htmlBuffer = "";
            ...
            if (evt.type === 'stdout' || evt.type === 'stderr') {
                 ...
                 lines.forEach((line: string) => {
                    if (line) {
                      ...
                      // Detect HTML table tags from table1
        ...
        // Also check result value if it's a string containing HTML
        if (!htmlBuffer && result.value) {
            try {
              const valStr = await result.value.toString();
              if (valStr.trim().startsWith('<') && valStr.trim().includes('table')) {
                eventBus.dispatch('r-html-created', { html: valStr });
              }
            } catch(e) {}
        }
  ...
  useEffect(() => {
    // Sync UI with existing engine status
