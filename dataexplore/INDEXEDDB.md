...
### 4. Database Seeding
On the first run (or after a "Clear Browser Data" action), the application automatically detects the absence of settings and datasets. It then hydrates the database with the `INITIAL_DATASETS` (e.g., `sample_study.csv`) and creates the default `appSettings` singleton.
