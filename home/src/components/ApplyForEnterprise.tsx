...
export default function ApplyForEnterprise() {
  ...
  useEffect(() => {
    const count = formData.message.trim().split(/\s+/).filter(word => word.length > 0).length;
    setWordCount(count);
    
    // Clear error if requirement met while typing
    if (count >= MIN_WORD_COUNT && errors.message) {
  ...
  const validateForm = () => {
    ...
    if (wordCount < MIN_WORD_COUNT) {
      newErrors.message = `Please provide more detail. (${wordCount}/${MIN_WORD_COUNT} words)`;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/send-enterprise-request.php`, {
        method: 'POST',
        headers: {
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div>
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <motion.div 
                      className={`text-xs font-semibold px-2 py-1 rounded ${wordCount >= MIN_WORD_COUNT ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}
                      animate={{ scale: wordCount >= MIN_WORD_COUNT ? [1, 1.1, 1] : 1 }}
                    >
                  ...
                  <div className="relative">
                    ...
                    <textarea
                      ...
                      placeholder={`Please describe your requirements in detail (minimum ${MIN_WORD_COUNT} words). Include information about:
- Your research goals
...
- Timeline for implementation`}
                  ...
                  {/* Progress bar */}
                  <div className="w-full h-1 bg-gray-100 mt-0">
                    <motion.div 
                      className={`h-full ${wordCount >= MIN_WORD_COUNT ? 'bg-emerald-500' : 'bg-blue-500'}`}
                      initial={{ width: 0 }}
                      animate={{ width: `${Math.min((wordCount / MIN_WORD_COUNT) * 100, 100)}%` }}
