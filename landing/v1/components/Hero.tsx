...
const Hero: React.FC = () => {
  ...
  return (
    <section className="relative overflow-hidden min-h-[600px] lg:min-h-[700px] flex items-center bg-[#020617] py-12 lg:py-0">
      ...
      <div className="max-w-[1600px] mx-auto px-6 lg:px-12 py-12 lg:py-24 relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center w-full">
        
        {/* Left Content */}
        <div className="max-w-4xl text-center lg:text-left">
          <p className="text-xs font-bold tracking-[0.2em] text-blue-400 uppercase mb-6 flex items-center justify-center lg:justify-start gap-2">
            <span className="w-8 h-[1px] bg-blue-400"></span>
            {hero.label}
          </p>
          
          <h1 className="text-4xl md:text-5xl lg:text-7xl font-bold text-white leading-[1.1] mb-8 tracking-tight drop-shadow-2xl">
            {hero.titlePrefix} 
            <span className="block text-transparent bg-clip-text bg-blue-300 mt-2 lg:mt-0"> {hero.titleHighlight}</span>
          </h1>
          
          <p className="text-lg text-slate-300 mb-12 leading-relaxed max-w-xl font-light border-l-0 lg:border-l-2 border-slate-700 pl-0 lg:pl-6 mx-auto lg:mx-0">
            {hero.description}
          </p>
          
          {/* Dark Glass Card */}
          <div className="bg-slate-900/40 backdrop-blur-sm p-8 border border-white/10 relative group max-w-lg mx-auto lg:mx-0">
            ...
            <a 
              ...
              className="relative z-10 bg-white text-slate-950 px-8 py-3 font-bold text-sm hover:bg-blue-50 transition-colors w-auto inline-block mt-4"
            >
              {hero.cta}
            </a>
          </div>
        ...
        {/* Right Area - Placeholder for visual balance - hidden on small mobile if needed, but keeps layout balanced on larger screens */}
