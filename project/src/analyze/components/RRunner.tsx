...
import { FileText, FunctionSquare, Package, ChevronLeft, ChevronRight, Brush, Download, ArrowUp, ArrowDown, ChevronsUpDown, Sparkles, Settings, HelpCircle, Play, Loader } from 'lucide-react';
...
export function RRunner({ consoleOutput, environment, tableData, plots, packages, editorCode, onClearPlots, onClearTable, onClearConsole, onViewObject, runCode, isLoading }: RRunnerProps) {
...
const ConsolePanel = ({ output, onClear, runCode, onRunScript, isLoading }: { output: ConsoleOutput[], onClear: () => void, runCode: (code: string) => void, onRunScript: () => void, isLoading: boolean }) => {
            ...
            <div className="absolute top-1 right-1 z-10 flex items-center space-x-1">
                ...
                {output.length > 0 && (
                    <button
                        ...
                        className="p-1.5 bg-gray-200 text-gray-600 rounded-full hover:bg-gray-300 hover:text-gray-800 transition-colors"
                        title="Clear console"
    ...
    <div className="h-full overflow-y-auto p-4 text-xs text-gray-700 space-y-4">
        <div>
            ...
            <ul className="list-disc list-inside space-y-1">
                ...
                <li><span className="font-mono bg-gray-200 px-1 rounded">Left/Right Arrow</span> (in Plots tab): Switch between plots.</li>
            </ul>
        </div>
        <div>
            ...
            <dl className="space-y-2">
                <div>
                    ...
                    <dd>In R, use the <code className="font-mono bg-gray-200 px-1 rounded">View(my_dataframe)</code> function. The data will appear in the "Tables" tab.</dd>
                </div>
                <div>
                    <dt className="font-semibold">Generating Plots:</dt>
                    <dd>Any standard plotting command (like <code className="font-mono bg-gray-200 px-1 rounded">plot()</code> or using <code className="font-mono bg-gray-200 px-1 rounded">ggplot2</code>) will generate a plot in the "Plots" tab.</dd>
                </div>
                <div>
                    <dt className="font-semibold">Environment:</dt>
                    <dd>The Environment tab shows all the variables and functions in your session. Click on a data frame to view it in the "Tables" tab, or click any other variable to print it to the console.</dd>
                </div>
            </dl>
        </div>
    </div>
