...
try {
    ...
    if (!$currentUser || $currentUser['team_role'] !== 'owner') {
        jsonResponse(['error' => 'Unauthorized: Only the team owner can cancel the subscription.'], 403);
    }
    ...
    // 4. Update Database
    $db->prepare("UPDATE teams SET status = 'canceling', cancel_at_period_end = 1 WHERE id = ?")->execute([$team['id']]);
    $db->prepare("UPDATE users SET cancel_at_period_end = 1 WHERE id = ?")->execute([$userId]);
    
    updateTeamStatus($db, $team['id']);

    jsonResponse([
        'success' => true,
        'message' => 'Team subscription will be canceled at the end of the billing period.'
    ]);
